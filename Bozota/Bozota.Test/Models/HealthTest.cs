...
namespace Bozota.Test.Models
{
    public class HealthTest
    {
        ...
        {
            var health = new Health(inPoints);
        ...
        {
            var health = new Health(inPoints, inMaxPoints);
            Assert.Equal(outPoints, health.Points);
            Assert.Equal(outMaxPoints, health.MaxPoints);
            Assert.Equal(outMinPoints, health.MinPoints);
            Assert.True(health.IsAlive);
        ...
        {
            var health = new Health(inPoints, inMaxPoints, inMinPoints);
            Assert.Equal(outPoints, health.Points);
            Assert.Equal(outMaxPoints, health.MaxPoints);
            Assert.Equal(outMinPoints, health.MinPoints);
            Assert.True(health.IsAlive);
        ...
        {
            var health = new Health(inPoint, maxPoints, minPoints);
            var result = health.Damage(damage);
        ...
        {
            var health = new Health(inPoint, maxPoints, minPoints);
            var result = health.Damage(damage, false);
        ...
        {
            var health = new Health(100, default, 1, true);
